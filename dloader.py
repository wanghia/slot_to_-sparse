...
class GCDataLoader:
    ...
    def _slot_parser(self, da):
        s = [x.strip() for x in da.split(':')]
    ...
    def _get_and_create_sign_id(self, slot, sign, is_descrete):
        ...
        if sign_id is None:
            ...
            self.signid2dis.append(is_descrete)
            self.cnt += 1

        return sign_id

    def load(self, filename, dense=False):
        ...
        for i, line in enumerate(content):
            ...
            label.append(int(s[0].split(' ')[0].strip()))
            # split by #, get data and split by space
            d = [x.strip() for x in s[1].split('#')[0].strip().split(' ')]

            for x in d:
                slot, sign_id, value = self._slot_parser(x)
                if sign_id != -1:
        ...
        if self.first_load:
            ...
            new_obj.data = tmp
            new_obj.label = np.array(label)

            return new_obj

    def write(self, filename):
        result = []
        for line, y in zip(self.data, self.label):
            l_re = []
            for sign_id, x in enumerate(line):
                is_dis = self.signid2dis[sign_id]
                if is_dis and x < 0.5:
                    continue
                slot = self.signid2slot[sign_id]
                sign = self.signid2sign[sign_id]

                if is_dis:
                    l_re.append("{}:{}".format(slot, sign))
                else:
                    l_re.append("{}:{}:{}".format(slot, sign, x))

            result.append("{} | ".format(y) + ' '.join(l_re))
